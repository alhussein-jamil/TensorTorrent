...
use sc_ir::{assert_schedule_valid, ExecutableArtifact, ExecutableSchedule};
use sc_runtime::{
...
impl NativeCompiledArtifact {
    ...
    fn execute(
        ...
        let dematerialize = dematerialize_callback.map(|callable| {
            ...
            }) as sc_runtime::DematerializeCallback
        });
        let materialize = materialize_callback.map(|callable| {
            let callable = Arc::new(callable);
            Arc::new(
                move |tensor_id: &str, meta: &sc_storage::SpillMeta, bytes: &[u8]| {
                    Python::with_gil(|py| {
                        crate::GIL_ACQUISITIONS.fetch_add(1, Ordering::Relaxed);
            ...
            ) as sc_runtime::MaterializeCallback
        });
        let parameter_load = parameter_load_callback.map(|callable| {
            ...
            }) as sc_runtime::ParameterLoadCallback
        });
        ...
        let cb: Option<RegionCallback> = region_callback.map(|callable| {
            let callable = Arc::new(callable);
            Arc::new(move |invocations: &[sc_runtime::RegionInvocation]| {
