...
use sc_backend_api::{Backend, EventStatus};
...
impl NativeVirtualBackend {
    ...
    fn transfer(&self, src: u64, dst: u64, bytes: usize, stream: &str) -> PyResult<u64> {
        let ev = self
            .inner
            .transfer(
                sc_backend_api::BufferHandle(src),
                sc_backend_api::BufferHandle(dst),
                bytes,
                StreamId::new(stream),
            )
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
    ...
    fn launch(&self, stream: &str) -> PyResult<u64> {
        let ev = self
            .inner
            .launch(
                sc_backend_api::ExecutableHandle(0),
                &[],
                &[],
                StreamId::new(stream),
            )
    ...
    fn query_event(&self, event: u64) -> PyResult<String> {
        let st = self
            .inner
            .query_event(sc_backend_api::EventHandle(event))
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
        Ok(match st {
    ...
    fn wait_event(&self, event: u64) -> PyResult<()> {
        ...
        Python::with_gil(|py| {
            py.allow_threads(|| {
                inner
                    .wait_event(sc_backend_api::EventHandle(event))
    ...
    fn write_bytes(&self, buffer: u64, data: &[u8]) -> PyResult<()> {
        self.inner
            .write_bytes(sc_backend_api::BufferHandle(buffer), data)
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))
    }
