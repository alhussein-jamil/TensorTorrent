...
use sc_ir::{ResourceId, TensorId};
use sc_memory::TensorMetadata;
...
pub type DematerializeCallback =
    Arc<dyn Fn(&str) -> Result<(sc_storage::SpillMeta, Vec<u8>), String> + Send + Sync>;

/// Contiguous host bytes → tensor registration for native activation reload.
pub type MaterializeCallback =
    Arc<dyn Fn(&str, &sc_storage::SpillMeta, &[u8]) -> Result<(), String> + Send + Sync>;
...
fn needs_python_io(inst: &sc_ir::Instruction, opts: &ExecuteOptions) -> bool {
...
fn execute_region_cb_inline(
    ...
    let mut ready: VecDeque<String> = remaining
        ...
        .filter_map(|(n, d)| if *d == 0 { Some(n.clone()) } else { None })
        .collect();
    let by_name: HashMap<&str, &sc_ir::Instruction> = schedule
        .instructions
        .iter()
    ...
    while !ready.is_empty() {
        ...
        if options.handle_release.is_some() {
            let mut release_insts: Vec<&sc_ir::Instruction> = Vec::new();
        ...
        if options.parameter_load.is_some() {
            let mut load_insts: Vec<&sc_ir::Instruction> = Vec::new();
...
fn execute_schedule_pooled(
    ...
    let by_name: HashMap<String, sc_ir::Instruction> = schedule
        .instructions
        .iter()
        .cloned()
        ...
        while inflight < options.max_inflight {
            ...
            if options.handle_release.is_some() {
                ...
                if !release_names.is_empty() {
                    let wave_insts: Vec<&sc_ir::Instruction> = release_names
                        .iter()
            ...
            if options.parameter_load.is_some() {
                ...
                if !load_names.is_empty() {
                    let wave_insts: Vec<&sc_ir::Instruction> =
...
fn enqueue_ready(
    name: &str,
    by_name: &HashMap<String, sc_ir::Instruction>,
...
fn is_batched_parameter_load(inst: &sc_ir::Instruction) -> bool {
...
fn order_key(inst: &sc_ir::Instruction) -> Option<String> {
...
fn attr_delay_s(inst: &sc_ir::Instruction, key: &str) -> Option<f64> {
    inst.attributes.get(key).and_then(|v| match v {
        sc_ir::AttrValue::Float(f) => Some(*f),
        sc_ir::AttrValue::Int(i) => Some(*i as f64),
        _ => None,
    })
}
...
fn run_instruction_body(
    inst: &sc_ir::Instruction,
    ctx: &NativeExecutionContext,
    residency: &sc_memory::ResidencyStore,
    region_cb: Option<&RegionCallback>,
    dry_run: bool,
    options: &ExecuteOptions,
    simulated: &mut bool,
    ...
    match inst.opcode {
        ...
        Opcode::Evict | Opcode::Release => {
            ...
            let idempotent = matches!(
                ...
                Some(sc_ir::AttrValue::Bool(true))
            );
...
fn is_batched_handle_release(inst: &sc_ir::Instruction) -> bool {
    match inst.opcode {
        Opcode::Release => true,
        Opcode::Evict => {
            let kind = inst.attr_str("kind").unwrap_or("");
...
fn native_activation_spill(
    ...
    for tid in &inst.inputs {
        ...
        let path = {
            ...
            sc_storage::write_activation_spill(&dir, &meta, &bytes)
                .map_err(|e| inst_err(inst, e.to_string()))?
        };
        ctx.with_storage(|st| {
...
fn native_activation_reload(
    ...
    for tid in &inst.inputs {
        ...
        let (meta, bytes) =
            sc_storage::read_activation_spill(&path).map_err(|e| inst_err(inst, e.to_string()))?;
        mat(tid.as_str(), &meta, &bytes).map_err(|e| inst_err(inst, e))?;
        ctx.with_storage(|st| {
            st.bytes_read += meta.nbytes;
            st.spills.remove(tid.as_str());
        });
        let _ = sc_storage::remove_activation_spill(&path);
        let tensor = TensorId::new(tid.as_str());
...
fn nbytes(inst: &sc_ir::Instruction, tensor: &str) -> u64 {
    inst.tensor_nbytes()
        .get(tensor)
        .copied()
...
fn load_notes(inst: &sc_ir::Instruction, residency: &sc_memory::ResidencyStore) -> String {
...
fn inst_err(inst: &sc_ir::Instruction, cause: String) -> Box<RuntimeError> {
    Box::new(RuntimeError::Instruction {
        instruction: inst.name.to_string(),
        opcode: inst.opcode.to_string(),
        region: inst.executable_ref.as_ref().map(|r| r.to_string()),
...
mod tests {
    ...
    use sc_ir::{Instruction, InstructionId, MemoryTier, RegionId};
    ...
    fn transfer_without_source_copy_fails() {
        let xfer = Instruction {
            ...
            transfer_backend: None,
            sync_required: false,
            stream_id: Some(sc_ir::StreamId::new("cpu::copy0")),
    ...
    fn wait_event_without_record_in_table_fails() {
        let wait = Instruction {
            ...
            transfer_backend: None,
            sync_required: false,
            stream_id: Some(sc_ir::StreamId::new("cpu::compute0")),
            copy_engine_id: None,
            link_id: None,
            io_queue_id: None,
            attributes: {
                ...
                m.insert(
                    "waits_for".into(),
                    sc_ir::AttrValue::String("never_recorded".into()),
                );
                m
            },
        };
    ...
    fn shared_context_survives_region_path() {
        let load = Instruction {
            ...
            transfer_backend: None,
            sync_required: false,
            stream_id: Some(sc_ir::StreamId::new("cpu::io0")),
            copy_engine_id: None,
            link_id: None,
            io_queue_id: None,
            attributes: IndexMap::new(),
    ...
    fn activation_spill_without_io_handler_fails_closed() {
        ...
        attrs.insert(
            "kind".into(),
            sc_ir::AttrValue::String("activation_spill".into()),
        );
