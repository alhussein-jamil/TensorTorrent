...
) -> Result<SimulationOutcome, sc_ir::CoreError> {
...
fn simulate_schedule_inner(
    schedule: &ExecutableSchedule,
    machine: &MachineModel,
) -> Result<SimulationResult, sc_ir::CoreError> {
            ...
            Opcode::Transfer => {
                ...
                for tid in unique_tensors(inst) {
                    ...
                    if !copies.contains_key(&(tid.clone(), src.to_owned())) {
                        return Err(sc_ir::CoreError::Validation(format!(
...
fn unique_tensors(inst: &sc_ir::Instruction) -> Vec<String> {
...
fn tensor_nbytes(inst: &sc_ir::Instruction, tensor: &str) -> u64 {
    ...
    for key in ["input_bytes", "output_bytes"] {
        if let Some(sc_ir::AttrValue::IntMap(m)) = inst.attributes.get(key) {
...
fn allocation_id(inst: &sc_ir::Instruction, tensor: &str, resource: &str) -> String {
    if let Some(sc_ir::AttrValue::StringMap(m)) = inst.attributes.get("allocation_ids") {
        if let Some(id) = m.get(tensor) {
            return id.clone();
        }
    }
    if let Some(sc_ir::AttrValue::Map(m)) = inst.attributes.get("allocation_ids") {
        if let Some(sc_ir::AttrValue::String(id)) = m.get(tensor) {
            return id.clone();
        }
    }
...
fn attr_f64(v: &sc_ir::AttrValue) -> Option<f64> {
    match v {
...
fn attr_u64(v: &sc_ir::AttrValue) -> Option<u64> {
    match v {
        sc_ir::AttrValue::Int(i) if *i >= 0 => Some(*i as u64),
        sc_ir::AttrValue::Float(f) if *f >= 0.0 => Some(*f as u64),
        _ => None,
    }
}
...
mod tests {
    use super::*;
    use sc_ir::{Instruction, InstructionId, MemoryTier, RegionId, ResourceId, TensorId};
