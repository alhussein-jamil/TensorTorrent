...
use sc_backend_virtual::{VirtualBackend, VirtualBackendConfig};
use sc_memory::{AllocationTable, ResidencyStore};
use sc_storage::StreamingStore;
...
impl NativeExecutionContext {
    ...
    pub(crate) fn next_alloc_id(&self) -> sc_ir::AllocationId {
        let n = self.alloc_counter.fetch_add(1, Ordering::Relaxed);
        sc_ir::AllocationId::new(format!("ex-{}-{}", self.execution_id.as_u64(), n))
    }
            ...
            .insert(alloc_key, (resource.to_owned(), buffer_id))
        {
            if prev_buf != buffer_id {
                if let Some(be) = self.virtual_backends.lock().get(&prev_res) {
                    let _ = be.free(sc_backend_api::BufferHandle(prev_buf));
                }
            }
        }
    ...
    pub fn free_virtual_buffer_for_alloc(&self, allocation_id: &str, freed_bytes: u64) {
        ...
        let _ = be.free(sc_backend_api::BufferHandle(buffer_id));
    }
    ...
    fn drop(&mut self) {
        ...
        for (resource, buf_id) in pending {
            if let Some(be) = self.virtual_backends.lock().get(&resource) {
                let _ = be.free(sc_backend_api::BufferHandle(buf_id));
            }
        }
    }
}
...
mod tests {
    ...
    fn final_alloc_release_frees_virtual_buffer() {
        use sc_backend_api::Backend;
        use sc_ir::{AllocationId, ResourceId, TensorId};
