...
device. Rather than assuming it does, TensorTorrent times the widest independent
        ...
        try:
            for candidate_workers, threads in _candidates(workers, process_threads):
                ...
                with inference_thread_pool(
                    max_workers=candidate_workers, thread_name_prefix="tensortorrent-measure"
                ) as pool:
                    run_parallel(pool)
...
def _time_full_graph(
    ...
    *,
    ...
    with torch.inference_mode():
        ...
        try:
            torch.set_num_threads(threads)
            with inference_thread_pool(max_workers=workers, thread_name_prefix="tensortorrent-full") as pool:
                run_parallel(pool)
