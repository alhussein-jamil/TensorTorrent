...
ENTRY_POINT_GROUP = "tensortorrent.backends"
...
def plugin_backends(*, refresh: bool = False) -> list[ExecutionBackend]:
    ...
    Plugins register an entry point in the ``tensortorrent.backends`` group.
    Set ``TENSORTORRENT_DISABLE_BACKEND_PLUGINS=1`` for hermetic deployments.
    """
    global _PLUGIN_FACTORIES
    if os.environ.get("TENSORTORRENT_DISABLE_BACKEND_PLUGINS", "").lower() in {"1", "true", "yes"}:
        return []
    with _LOCK:
        if refresh:
