...
class CpuBackend(ExecutionBackend):
    ...
    def discover_devices(self) -> ResourceGraph:
        ...
        # Use budget resolver for per-node allowed bytes; capacity stays physical.
        n_nodes = max(1, len(numa_nodes))
        host_budget = _budget.resolve_host_memory_budget()
        host_allowed_per_node = max(0, host_budget.allowed_bytes // n_nodes)
        mem_total_per_node = vm.total // n_nodes

        wsl2_attrs: dict[str, str] = {}
        if _budget.is_wsl2():
            wsl2_attrs["wsl2"] = "true"

        for node in numa_nodes:
            mem_name = f"numa_ram_{node}"
            # allocatable = allowed / n_nodes from budget resolver (cgroup-aware)
            # capacity = physical total / n_nodes (for display)
            graph.add_memory(
                MemoryResource(
                    id=ResourceId(ResourceKind.MEMORY, mem_name),
                    memory_class=MemoryClass.NUMA_RAM,
                    capacity_bytes=mem_total_per_node,
                    allocatable_bytes=host_allowed_per_node,
                    numa_node=node,
                    attributes={
                        "source": host_budget.source.kind,
                        "budget_source": host_budget.source.kind,
                        "budget_detail": host_budget.source.detail,
                        "budget_reserved_bytes": str(host_budget.reserved_bytes),
                        **wsl2_attrs,
                    },
                )
            )
            pinned_name = f"pinned_host_{node}"
            # Pinned host pool sized from allowed (25%/20% shape preserved)
            pinned_cap = int(host_allowed_per_node * 0.25)
            pinned_alloc = int(host_allowed_per_node * 0.20)
            graph.add_memory(
                MemoryResource(
                    id=ResourceId(ResourceKind.MEMORY, pinned_name),
                    memory_class=MemoryClass.PINNED_HOST,
                    capacity_bytes=pinned_cap,
                    allocatable_bytes=pinned_alloc,
                    numa_node=node,
                    attributes={"budget_source": host_budget.source.kind, **wsl2_attrs},
                )
            )
