...
The backend is capability-gated: importing StreamCompiler on a host without an
...
class XpuBackend(ExecutionBackend):
    ...
    def discover_devices(self) -> ResourceGraph:
        ...
        for index in range(self._device_count()):
            ...
            allocatable = int(total * 0.9) if total > 0 else 0
            graph.add_memory(
                MemoryResource(
                    ...
                    attributes={"backend": self.backend_id, "index": index, "capacity_known": total > 0},
                )
            )
            graph.add_compute(
                ComputeResource(
