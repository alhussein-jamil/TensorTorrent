...
impl PackManifest {
    ...
    pub fn validate(&self) -> StorageResult<()> {
        ...
        for (offset, end, name) in ranges {
            ...
            if offset < end && active.is_none_or(|(active_end, _)| end > active_end) {
...
mod tests {
    ...
    #[test]
    fn pread_roundtrip() {
        let dir = std::env::temp_dir().join("tt-pack-test");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("data.bin");
        let payload = b"hello-tensortorrent";
        {
