...
use tt_backend_api::{
    Backend, BackendCapabilities, BackendError, BackendResourceView, BackendResult, BufferHandle,
...
impl Backend for VirtualBackend {
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            simulated: true,
            resources: vec![BackendResourceView {
                resource_id: self.config.name.clone(),
                memory_domain_id: format!("{}_mem", self.config.name),
                numa_node: None,
                compute_streams: 2,
                copy_streams: self.config.max_copy_engines,
                copy_engines: self.config.max_copy_engines,
                peer_access: Vec::new(),
    ...
    fn memory_report(&self) -> tt_backend_api::BackendMemoryReport {
        tt_backend_api::BackendMemoryReport {
            device_used_bytes: *self.used_bytes.lock(),
            device_total_bytes: self.config.memory_bytes,
            host_pinned_bytes: 0,
            live_allocations: self.buffers.lock().len() as u64,
        }
    }

    fn health(&self) -> tt_backend_api::BackendHealth {
        tt_backend_api::BackendHealth {
            healthy: !self.shutdown.load(Ordering::Acquire),
            detail: if self.shutdown.load(Ordering::Acquire) {
                "shutdown".into()
            } else {
                "ok".into()
            },
...
mod tests {
    ...
    fn ordered_stream_serializes() {
        ...
        // Widened lower bound from 0.035s to 0.01s (~3x looser) for determinism on
        // 2-core CI hosts where thread scheduling jitter can cut into the sleep.
    ...
    fn separate_streams_overlap() {
        ...
        // Widened upper bound from 0.055s to 0.165s (~3x) for determinism on
        // 2-core CI hosts where thread-wakeup latency can inflate wall time.
        assert!(t0.elapsed().as_secs_f64() < 0.165);
    }
