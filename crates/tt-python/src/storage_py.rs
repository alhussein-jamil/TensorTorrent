...
#[pyclass(module = "tensortorrent._native", name = "NativeStreamingStore")]
...
pub fn write_activation_spill(
    ...
    let meta = tt_storage::SpillMeta {
        dtype: dtype.to_owned(),
        shape,
        nbytes: bytes.len() as u64,
    };
    let path = tt_storage::write_activation_spill(Path::new(dir), &meta, bytes)
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
...
pub fn remove_activation_spill(path: &str) -> PyResult<()> {
    tt_storage::remove_activation_spill(Path::new(path))
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))
}
