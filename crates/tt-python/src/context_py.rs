//! PyO3 wrapper around [`tt_runtime::NativeExecutionContext`].
...
use tt_runtime::NativeExecutionContext;

/// One authoritative native execution context per forward.
#[pyclass(module = "tensortorrent._native", name = "NativeExecutionContext")]
...
impl PyNativeExecutionContext {
    ...
    fn set_spill_budget_bytes(&self, bytes: u64) {
        self.inner.set_spill_budget_bytes(bytes);
    }

    /// Configure the progress-stall watchdog in seconds (0 disables).
    fn set_stall_timeout_secs(&self, secs: f64) {
        self.inner.set_stall_timeout_secs(secs);
    }

    /// Enforce a hard byte ceiling for a resource on the real allocation path.
    fn set_resource_capacity(&self, resource: &str, bytes: u64) {
        self.inner.set_resource_capacity(resource, bytes);
    }
    ...
    fn virtual_buffer_from_bytes(&self, resource: &str, data: &[u8]) -> PyResult<u64> {
        ...
        let h = be
            .allocate(tt_ir::ResourceId::new(resource), data.len().max(1), 64)
    ...
    fn virtual_buffer_to_bytes<'py>(
        ...
        let be = self.inner.virtual_backend(resource);
        let bytes = be
            .read_bytes(tt_backend_api::BufferHandle(buffer))
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    ...
    fn set_virtual_backend_config(
        ...
        let mut cfg = tt_backend_virtual::VirtualBackendConfig {
