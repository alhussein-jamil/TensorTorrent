...
use std::sync::Arc;
use tt_backend_api::{Backend, EventStatus};
use tt_backend_virtual::{VirtualBackend, VirtualBackendConfig};
use tt_ir::{ResourceId, StreamId};

#[pyclass(module = "tensortorrent._native", name = "NativeVirtualBackend")]
...
impl NativeVirtualBackend {
    ...
    fn transfer(&self, src: u64, dst: u64, bytes: usize, stream: &str) -> PyResult<u64> {
        let ev = self
            .inner
            .transfer(
                tt_backend_api::BufferHandle(src),
                tt_backend_api::BufferHandle(dst),
                bytes,
                StreamId::new(stream),
            )
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
    ...
    fn launch(&self, stream: &str) -> PyResult<u64> {
        let ev = self
            .inner
            .launch(
                tt_backend_api::ExecutableHandle(0),
                &[],
                &[],
                StreamId::new(stream),
            )
    ...
    fn query_event(&self, event: u64) -> PyResult<String> {
        let st = self
            .inner
            .query_event(tt_backend_api::EventHandle(event))
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
        Ok(match st {
    ...
    fn write_bytes(&self, buffer: u64, data: &[u8]) -> PyResult<()> {
        self.inner
            .write_bytes(tt_backend_api::BufferHandle(buffer), data)
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))
    }
