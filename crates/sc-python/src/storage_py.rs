...
use sc_storage::{ChunkCache, PackManifest, PackReader, StreamingStore, TensorEntry};
...
pub fn write_activation_spill(
    ...
    let meta = sc_storage::SpillMeta {
        dtype: dtype.to_owned(),
        shape,
        nbytes: bytes.len() as u64,
    };
    let path = sc_storage::write_activation_spill(Path::new(dir), &meta, bytes)
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
...
pub fn read_activation_spill(py: Python<'_>, path: &str) -> PyResult<PyObject> {
    let (meta, bytes) = sc_storage::read_activation_spill(Path::new(path))
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
    let d = PyDict::new(py);
...
pub fn remove_activation_spill(path: &str) -> PyResult<()> {
    sc_storage::remove_activation_spill(Path::new(path))
        .map_err(|e| PyRuntimeError::new_err(e.to_string()))
}
