...
fn needs_python_io(inst: &tt_ir::Instruction, opts: &ExecuteOptions) -> bool {
...
fn is_batched_parameter_load(inst: &tt_ir::Instruction) -> bool {
...
fn is_native_launch(inst: &tt_ir::Instruction) -> bool {
...
fn order_key(inst: &tt_ir::Instruction) -> Option<String> {
...
fn attr_delay_s(inst: &tt_ir::Instruction, key: &str) -> Option<f64> {
...
fn is_batched_handle_release(inst: &tt_ir::Instruction) -> bool {
    match inst.opcode {
        Opcode::Release => true,
        Opcode::Evict => {
            let kind = inst.attr_str("kind").unwrap_or("");
...
fn nbytes(inst: &tt_ir::Instruction, tensor: &str) -> u64 {
    inst.tensor_nbytes()
        .get(tensor)
        .copied()
...
fn load_notes(inst: &tt_ir::Instruction, residency: &tt_memory::ResidencyStore) -> String {
...
fn inst_err(inst: &tt_ir::Instruction, cause: String) -> Box<RuntimeError> {
    Box::new(RuntimeError::Instruction {
        instruction: inst.name.to_string(),
        opcode: inst.opcode.to_string(),
        region: inst.executable_ref.as_ref().map(|r| r.to_string()),
