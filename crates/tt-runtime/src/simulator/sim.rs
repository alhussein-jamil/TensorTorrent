...
) -> Result<SimulationOutcome, tt_ir::CoreError> {
...
fn simulate_schedule_inner(
    schedule: &ExecutableSchedule,
    machine: &MachineModel,
) -> Result<SimulationResult, tt_ir::CoreError> {
            ...
            Opcode::Transfer => {
                ...
                for tid in unique_tensors(inst) {
                    ...
                    if !copies.contains_key(&(tid.clone(), src.to_owned())) {
                        return Err(tt_ir::CoreError::Validation(format!(
...
fn unique_tensors(inst: &tt_ir::Instruction) -> Vec<String> {
...
fn tensor_nbytes(inst: &tt_ir::Instruction, tensor: &str) -> u64 {
    ...
    for key in ["input_bytes", "output_bytes"] {
        if let Some(tt_ir::AttrValue::IntMap(m)) = inst.attributes.get(key) {
...
fn allocation_id(inst: &tt_ir::Instruction, tensor: &str, resource: &str) -> String {
    if let Some(tt_ir::AttrValue::StringMap(m)) = inst.attributes.get("allocation_ids") {
        if let Some(id) = m.get(tensor) {
            return id.clone();
        }
    }
    if let Some(tt_ir::AttrValue::Map(m)) = inst.attributes.get("allocation_ids") {
        if let Some(tt_ir::AttrValue::String(id)) = m.get(tensor) {
            return id.clone();
        }
    }
...
fn attr_f64(v: &tt_ir::AttrValue) -> Option<f64> {
    match v {
...
fn attr_u64(v: &tt_ir::AttrValue) -> Option<u64> {
    match v {
        tt_ir::AttrValue::Int(i) if *i >= 0 => Some(*i as u64),
        tt_ir::AttrValue::Float(f) if *f >= 0.0 => Some(*f as u64),
        _ => None,
    }
}
...
mod tests {
    use super::*;
    use tt_ir::{Instruction, InstructionId, MemoryTier, RegionId, ResourceId, TensorId};
