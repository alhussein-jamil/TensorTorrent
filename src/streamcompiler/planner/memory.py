    ...
    for tid in ordered:
        ...
        for i, (other_tid, offset, other_size, _, _op, ol) in enumerate(list(live_allocs)):
            ...
            if produced is not None and ol is not None and produced > ol and other_size >= size:
                ...
                break
        if not reused:
            plan.allocations.append(Allocation(tid, mem_name, cursor, size))
            live_allocs.append((tid, cursor, size, cursor + size, produced, last_use))
            cursor += size
        plan.peak_bytes[mem_name] = max(plan.peak_bytes.get(mem_name, 0), cursor)
        _ = free_intervals
    return plan
