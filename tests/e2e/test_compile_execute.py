"""End-to-end equivalence tests: TensorTorrent output must equal eager PyTorch."""
...
def assert_matches_eager(model: nn.Module, args: tuple[object, ...]) -> tt.CompiledModule:
    model = model.eval()
    with torch.no_grad():
        expected = model(*args)
    compiled = tt.compile(model, args, devices="auto")
...
def test_branching_model_matches_eager() -> None:
    ...
    if compiled.specialized.validation.get("fused_after_sequential_decision"):
        ...
        # Fused resident single-region plans take the direct path by default.
...
def test_identity_model_returns_its_input() -> None:
    ...
    x = torch.randn(2, 3)
    compiled = tt.compile(Identity().eval(), (x,))
...
def test_cpu_only_execution_uses_cpu_backend() -> None:
    compiled = tt.compile(Mlp().eval(), (torch.randn(2, 16),), devices="cpu")
...
def test_artifacts_and_visualization(tmp_path: Path) -> None:
    ...
    compiled = tt.compile(
        model,
        (x,),
        config=tt.CompileConfig(objective=tt.Objective.LATENCY, profile_level="coarse"),
...
def test_call_fast_path_is_used_only_when_the_spec_allows_it() -> None:
    ...
    flat = tt.compile(MultiInput().eval(), (torch.randn(2, 8), torch.randn(2, 4), torch.randn(2, 8)))
    assert flat.program.positional_tensor_arity == 3
    assert flat.program.single_output is True

    structured = tt.compile(StructuredOutputs().eval(), (torch.randn(2, 8),))
