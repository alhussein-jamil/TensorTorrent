...
def test_plan_collectives_marks_host_staged_for_mixed_devices() -> None:
    ...
    assert plans
    assert plans[0].backend_id in {"host_staged", "gloo"}


def test_plan_collectives_does_not_invent_ops_absent_from_the_ir() -> None:
    graph = HeterogeneousGraph(name="c")
    plans = plan_collectives(graph, ResourceGraph(fingerprint="x"), ("cuda_gpu_0", "cuda_gpu_1"))
    assert plans == []
